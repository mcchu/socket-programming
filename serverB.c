...
#include <arpa/inet.h>
#define UDP "22882" // the port client will be connecting to 
...
//creation of socket and networking drived from beej "client" example
    ...
    if ((rv = getaddrinfo(localhost, CLIENT_PORT, &hints, &servinfo)) != 0) {	//PORT --> CLIENT_PORT
    ...
    printf("%s", CLIENT_PORT); printf(" and IP address %s" , s); printf("\n");
...
// UDP listener code drived from beej datagram example
    ...
    printf("\nThe server B has received the network toplogy from the Client with UDP port number %d and IP address %s as follows:\n\n",
	 sock_udp,
    inet_ntop(client_addr.ss_family, get_in_addr((struct sockaddr *)&client_addr), q, sizeof q));
    //printf("UDP Listener: packet is %d bytes long\n", num_udp_bytes);
    buf[num_udp_bytes] = '\0';
    //printf("UDP Listener: packet contains \"%s\"\n", buf);
    close(sock_udp);
    int count;
    ...
    printf("\nFor this connection with Client, The Server B has UDP port number %s and IP address ", UDP);
    for(ii = 0; addr_list[ii] != NULL; ii++) {printf("%s ", inet_ntoa(*addr_list[ii]));} 
    printf("\n\n");
