...
#include <arpa/inet.h>
#define UDP "23882" // the port client will be connecting to 
...
//creation of socket and networking drived from beej "client" example
    ...
    inet_ntop(p->ai_family, get_in_addr((struct sockaddr *)p->ai_addr),
            s, sizeof s);
    //printf("The Server %c" , server_name);printf(": connecting to %s\n", s);
...
// UDP listener code drived from beej datagram example
    ...
    printf("\nThe server C has received the network toplogy from the Client with UDP port number %d and IP address %s as follows:\n\n",
	 sock_udp,
    ...
    printf("\nFor this connection with Client, The Server C has UDP port number %s and IP address ", UDP);
    for(ii = 0; addr_list[ii] != NULL; ii++) {printf("%s ", inet_ntoa(*addr_list[ii]));} 
    printf("\n\n");
