...
#include <arpa/inet.h>
#define UDP "21882" // the port client will be connecting to 
...
// UDP listener code drived from beej datagram example
    ...
    printf("\nThe server A has received the network toplogy from the Client with UDP port number %d and IP address %s as follows:\n\n",
	 sock_udp,
    ...
    printf("\nFor this connection with Client, The server A has UDP port number %s and IP address ", UDP);
    for(ii = 0; addr_list[ii] != NULL; ii++) {printf("%s ", inet_ntoa(*addr_list[ii]));} 
    printf("\n\n");
    return 0;
}
